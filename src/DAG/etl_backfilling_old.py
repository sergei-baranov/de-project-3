...
with DAG(
    ...
    load_user_order_log = PythonOperator(
        ...
        op_kwargs={
            'business_dt': dag.params['business_dt'],
            'filebase': 'user_orders_log',  # not b but user_orders_log.csv (orderS)
            'pg_table': 'staging.' + b,
            'conn_obj': pg_conn
        },
        dag=dag)
...
set_gen_report_task >> delay_task >> get_report_task >> get_files_task >> \
    [load_customer_research, load_user_order_log, load_user_activity_log] >> \
    truncate_mart >> update_dimensions >> update_facts
    # , load_price_log
